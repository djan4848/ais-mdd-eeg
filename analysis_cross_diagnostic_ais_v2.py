"""
AIS_rest cross-diagnostic comparison — TDBRAIN  (v2 CORRECTED 2026-06-04)
MDD values corrected: original used HP-only filter (d=2.024, artefact).
v2 uses bandpass 1-40Hz for all groups consistently (d=0.527).
CTL, ADHD, SMC, OCD: unchanged.
"""
...
# v2 FIX: replace MDD values from HP-only pipeline with bandpass-consistent values
_DEP_DECO_TDBRAIN = Path("/media/neuraldyn/PortableSSD/DEP_DECO/results/TDBRAIN/nais_all_subjects_TDBRAIN.csv")
_df_corr = pd.read_csv(_DEP_DECO_TDBRAIN)
_fcz     = _df_corr[_df_corr['channel'] == 'FCz']
_mdd_corr = _fcz[_fcz['group'].isin(['mdd_current','mdd_past'])][['subject_id','AIS_emp']].copy()
_mdd_corr.columns = ['subject_id', 'ais_rest']
_mdd_corr['group']    = 'MDD'
_mdd_corr['remitter'] = np.nan  # not used in this script

df_existing = pd.read_csv(TDBRAIN / "derivatives/tdbrain_ais_mdd_ctl.csv")
# Replace MDD rows with corrected values (CTL rows are already correct)
_ctl_rows = df_existing[df_existing['group']=='CTL'].copy()
df_existing = pd.concat([_ctl_rows, _mdd_corr], ignore_index=True)
AIS_COL = 'ais_rest'
...
df_all.to_csv(TDBRAIN / "derivatives/tdbrain_ais_all_disorders_v2.csv", index=False)
print("Saved: tdbrain_ais_all_disorders_v2.csv  (MDD corrected)")
...
out_png = ASSETS / "cross_diagnostic_ais_rest_v2.png"
out_corrected = Path("/media/neuraldyn/PortableSSD/DEP_DECO/figures/corrected/FigureS1_cross_diagnostic_v2.png")
fig.savefig(out_png, dpi=200, bbox_inches='tight', facecolor='white')
fig.savefig(out_corrected, dpi=200, bbox_inches='tight', facecolor='white')
...
df_res.to_csv(TDBRAIN / "derivatives/tdbrain_cross_diagnostic_results_v2.csv", index=False)
print("\n✅ Results saved: tdbrain_cross_diagnostic_results_v2.csv")
print("\nIMPLICATIONS:")
