"""
SCRIPT 14v2: MANUSCRIPT FIGURE — CORRECTED 2026-06-04
Corrects TDBRAIN preprocessing error (CTL bandpass vs MDD HP-only).
MODMA has NO error — value updated to forensic-audited 0.328.

Changes from v1:
  1. TDBRAIN data: loaded from DEP_DECO correct CSV (bandpass for both groups)
     Old: tdbrain_ais_mdd_ctl.csv (MDD from HP-only pipeline → d=2.024 artefact)
     New: DEP_DECO/results/TDBRAIN/nais_all_subjects_TDBRAIN.csv (d=0.527)
  2. Panel F evidence map:
     TDBRAIN: d=2.024 → 0.527, p=.001 → .006  (only correction)
     MODMA rest: 0.319 → 0.328 (forensic audit value; original had minor rounding diff)
  3. Panel F x-axis: 2.9 → 1.6 (scaled to new data range)
All other panels: UNCHANGED. Style, colors, fonts: IDENTICAL to v1.
"""
import numpy as np
import pandas as pd
import matplotlib
...
# CORRECTION v2: load from DEP_DECO bandpass-consistent pipeline
# Original tdbrain_ais_mdd_ctl.csv had CTL=bandpass, MDD=HP-only → d=2.024 (artefact)
# Correct source: same bandpass 1-40Hz for both groups → d=0.527
_tdb_correct = Path("/media/neuraldyn/PortableSSD/DEP_DECO/results/TDBRAIN/nais_all_subjects_TDBRAIN.csv")
_df_tdb_all  = pd.read_csv(_tdb_correct)
_df_tdb_fcz  = _df_tdb_all[_df_tdb_all['channel'] == 'FCz']
ctl_tdb   = _df_tdb_fcz[_df_tdb_fcz['group'] == 'ctl']['AIS_emp'].dropna()
mdd_tdb   = _df_tdb_fcz[_df_tdb_fcz['group'].isin(['mdd_current','mdd_past'])]['AIS_emp'].dropna()
...
findings = [
    ...
    ('TDBRAIN\n(rest, N=168)',             0.527, 0.006,  'Rest', True),   # CORRECTED (was 2.024)
    ('MODMA\n(rest, N=53)',               0.328, 0.258,  'Rest', False),  # forensic-audited value (DEP_DECO stats_all_MODMA.csv E6)
...
for i, (label, d_v, p_v, dtype, sig) in enumerate(findings):
    ...
    ax.text(d_v + 0.04, y, pval_stars(p_v), va='center', fontsize=8,
            color='black' if sig else COL_GRAY)
    ax.text(d_v + 0.17, y, f'd={d_v:.2f}', va='center', fontsize=7.5, color=COL_GRAY)
...
ax.set_xlim(-0.1, 1.60)  # CORRECTED: scaled to actual data range (was 2.9)
...
out_png    = ASSETS / "Figure1_manuscript_v2.png"
out_pdf    = ASSETS / "Figure1_manuscript_v2.pdf"
out_corrected_png = Path("/media/neuraldyn/PortableSSD/DEP_DECO/figures/corrected/Figure1_manuscript_v2.png")
out_corrected_pdf = Path("/media/neuraldyn/PortableSSD/DEP_DECO/figures/corrected/Figure1_manuscript_v2.pdf")
...
fig.savefig(out_corrected_png, dpi=300, bbox_inches='tight', facecolor='white', edgecolor='none')
fig.savefig(out_corrected_pdf,           bbox_inches='tight', facecolor='white', edgecolor='none')
print(f"✅ Corrected figure saved:")
print(f"   PNG (300 DPI): {out_png}")
print(f"   PDF (vector):  {out_pdf}")
print(f"   Also at: {out_corrected_png}")

print(f"\n=== PANEL CONTENT SUMMARY ===")
print(f"A: AIS_pre task     d={d_a:.3f}, p={p_a:.4f}  (N={len(ctl_pre)}+{len(mdd_pre)})")
print(f"B: AIS_rest rest    d={d_3478:.3f} (ds003478) / {d_tdb:.3f} (TDBRAIN — CORRECTED)")
...
print(f"\nCORRECTIONS APPLIED:")
print(f"  TDBRAIN: d=2.024 → {d_tdb:.3f}  (filter inconsistency corrected)")
print(f"  MODMA rest: 0.319 → 0.328  (forensic-audited value; MODMA had no pipeline error)")
print(f"  Panel F x-axis: 2.9 → 1.6  (rescaled)")
